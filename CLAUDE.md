...
- **Uploaded images — both `local` and `s3` providers — are served by `app/uploads/[...path]/route.ts`, never by Next's static `public/` serving and never by pointing the browser straight at a bucket.** `next start` computes its list of servable `public/` files once at server boot and never rescans the filesystem afterward — a file that appears after boot (every runtime upload, by definition) 404s forever under plain static serving no matter how long it's actually on disk. This bit a real deployment: the file persisted correctly on a Railway volume, but the already-running server still 404'd it. `next dev` never reproduces this (it watches the filesystem), so this class of bug only shows up under `next start`/production — test image uploads in production mode (`pnpm build && pnpm start`), not just `pnpm dev`, before considering an image-pipeline change verified. The route calls `getImageStorage().read(storageKey)` — a required `read()` method on the `ImageStorage` interface (`lib/uploads/storage/types.ts`) — rather than reading the filesystem directly, so it works identically for both providers. If this route handler or the `/uploads/` URL prefix is ever changed, also update `LocalImageStorage.getUrl()`/`S3ImageStorage.getUrl()` and `extractStorageKeyFromUrl()` (`lib/uploads/`) — they must all agree on the prefix.
...
- **S3-compatible bucket storage provider**: `STORAGE_PROVIDER=s3` (`lib/uploads/storage/s3-image-storage.ts`) is now implemented alongside `local`, using `@aws-sdk/client-s3` against a Railway bucket (any S3-compatible endpoint works the same way — Railway doesn't have a proprietary storage SDK, its buckets just speak the standard S3 API, same as R2/MinIO/Spaces/Backblaze). Config: `S3_ENDPOINT`, `S3_BUCKET`, `S3_REGION`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` — **all server-only, no `NEXT_PUBLIC_` prefix on any of them**, read only inside `server-only`-guarded files.
  ...
  - `app/uploads/[...path]/route.ts` serves both providers uniformly now (see above) — switching `STORAGE_PROVIDER` only changes where *new* uploads go; it does not move, delete, or break access to files a previous provider already stored.
  ...
  - **Known limitation, not yet resolved**: this local dev checkout's `public/images/` did not contain the files the live database's `Category`/`Banner` rows actually reference (those live on whatever server is really deployed against the shared `DATABASE_URL`, e.g. a Railway volume this checkout has no access to) — so no automated migration of pre-existing images into the bucket has been done. New uploads go to the bucket; old ones keep being served by whatever previously served them (unaffected by this change) until someone re-uploads them through the admin panel, or runs an equivalent migration script from the environment where those files actually live.
