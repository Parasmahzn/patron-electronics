...
> **Implementation status: DONE (+ extended beyond spec).** Beyond the items above: a 10-minute idle timeout (server-side authoritative check in `getAdminSession()` + a client-side warning modal, `components/admin/IdleTimeoutWatcher.tsx`, that redirects to the public homepage — not `/admin/login` — if it actually expires unattended), a self-service account menu (`components/admin/AdminUserMenu.tsx`, topbar avatar dropdown) letting the signed-in admin change their own password or profile picture without touching `.env`/reseeding, and a login flow hardened against IP-spoofed rate-limit bypass and email-enumeration-by-timing. Full detail in section 104.
...
> - `lib/uploads/storage/` — the abstraction: an `ImageStorage` interface (`upload`/`delete`/`getUrl`/`exists`/`read`), implemented by `S3ImageStorage` against any S3-compatible bucket (config via `S3_*` env vars). **S3-only — there is no local-disk storage provider**; an earlier `LocalImageStorage` implementation was removed once S3 was in place, so all environments (including local dev) require a working bucket. Destinations: `products`, `categories`, `services`, `avatars`, `banners` (`lib/uploads/upload.constants.ts`).
> - **Served via `app/uploads/[...path]/route.ts`, never a bucket-direct URL** — the bucket doesn't need to support public read (several providers, e.g. Railway Storage Buckets, don't). This route calls `getImageStorage().read()` (an authenticated `GetObjectCommand`) fresh on every request instead. Full detail/history in section 104.
> - `public/seed/` (static, git-tracked, generated by `prisma/seed.ts`) is the only image content that ships with the repo/build — all real uploads live only in the bucket, never on the app's own disk. See [README.md § Image Handling](README.md#image-handling).
...
> **Implementation note**: no `AUTH_SECRET` — admin sessions are random tokens stored (hashed) in the database, not signed/JWT-based, so there's nothing for a secret to sign. See section 104 for the full, current `.env.example` contents (S3 is the only image storage option; there's no `STORAGE_PROVIDER` toggle).
...
- **`public/`**: contains only real, referenced content — the `create-next-app` default scaffold SVGs (`next.svg`, `vercel.svg`, `file.svg`, `globe.svg`, `window.svg`) were removed as genuinely unused (nothing in the codebase ever referenced them). Do not recreate them. `public/seed/products/` (the 26 seed-generated placeholder SVGs, static and git-tracked — do not point runtime uploads here) is the only real asset tree in `public/`. There is deliberately no `public/images/` — all real uploads live only in the S3 bucket, never on the app's own disk (see the storage-provider bullet below).
- **Uploaded images are served by `app/uploads/[...path]/route.ts`, never by pointing the browser straight at the bucket.** The route calls `getImageStorage().read(storageKey)` — a required `read()` method on the `ImageStorage` interface (`lib/uploads/storage/types.ts`), implemented as an authenticated `GetObjectCommand` — rather than any filesystem/static serving. If this route handler or the `/uploads/` URL prefix is ever changed, also update `S3ImageStorage.getUrl()` and `extractStorageKeyFromUrl()` (`lib/uploads/`) — they must agree on the prefix. (An earlier `local` provider wrote to `public/images/` and had to work around `next start` never rescanning `public/` after boot — moot now that S3 is the only provider, but if a local/disk provider is ever reintroduced, re-read the git history around this note before assuming static serving works for runtime-written files.)
- **A banner (or any large) image upload returning a bare, message-less `500` is very likely NOT a storage-provider problem — check `next.config.ts`'s `experimental.serverActions.bodySizeLimit` first.** Next.js Server Actions default to a 1MB request body limit; `uploadImageAction`/`createBannerAction`/`updateBannerAction` (and equivalents) all wrap their real work in try/catch and return a typed `{error}` on failure, but a body over the limit is rejected by Next itself *before* that code ever runs, bypassing all of it and surfacing as a raw framework 500 with no useful message. `next.config.ts` sets `bodySizeLimit: '26mb'` (headroom above `UPLOAD_MAX_SIZE_BYTES` = 25MB in `lib/uploads/upload.constants.ts` for multipart framing overhead) specifically so this can't happen again — if `UPLOAD_MAX_SIZE_BYTES` is ever raised, raise this alongside it.
- **Admin idle timeout**: `AdminSession.lastActiveAt`, checked and refreshed (throttled) inside `getAdminSession()`; `ADMIN_IDLE_TIMEOUT_MINUTES`/`ADMIN_IDLE_WARNING_SECONDS` in `config/site.ts`. Server-side check is authoritative; `components/admin/IdleTimeoutWatcher.tsx` is the client-side half that actually forces a logout while an admin is idle on a static page (the server check alone only fires on the *next* request). The warning is a blocking modal (`components/ui/Dialog.tsx`) with a live countdown, not a dismissible corner toast — deliberate, since incidental mouse movement while the modal is up must not silently reset the timer; the admin has to explicitly click "Stay logged in" or "Sign out now". There are **two** distinct sign-out actions in `app/actions/auth.ts`, redirecting to different places on purpose: `logoutAction()` (topbar "Logout", and the warning modal's "Sign out now" — both a deliberate click by whoever's present) goes to `/admin/login`; `idleLogoutAction()` (the *auto*-triggered path when the countdown reaches zero with no response) goes to `/` instead, since an unattended session that actually timed out is unlikely to still be that admin at the keyboard.
- **Login hardened against two issues found in a security audit**: (1) `getClientIdentifier()` (`app/actions/auth.ts`) — used to key the DB-backed login lockout (`lib/auth/rate-limit.ts`, 5 attempts/15 min) — now trusts the *last* comma-separated entry of `X-Forwarded-For`, not the first. A proxy appends the address of whoever connected to *it* to the end of the header; for this app's single-hop deployment (browser → Railway's edge → this container) the last entry is the one Railway itself observed, while the first is client-supplied and was trivially spoofable — an attacker could previously defeat the lockout entirely by sending a different fake value on every attempt. If a second proxy (e.g. a CDN) is ever added in front of Railway, this needs to skip one additional trusted hop from the end. (2) `verifyLoginPassword()` (`lib/auth/password.ts`) always runs a real bcrypt comparison — against a fixed dummy hash when the submitted email doesn't match any admin, instead of short-circuiting straight to `false` — so response time can't be used to enumerate valid admin emails. `changePasswordAction` still uses the original `verifyPassword()` (an authenticated admin re-entering their own current password has no enumeration surface).
- **404 and error pages**: `app/not-found.tsx` renders a real 404 status with a message, then `router.replace('/')` after 3 seconds (plus an immediate manual `<Link>`, and a no-JS fallback). `app/error.tsx` (Next's root error boundary) has a "Go to homepage" `<Link>` alongside the existing "Try again"/`reset()` button — still logs server-side only, never renders error internals to the visitor. Both use ordinary `useRouter()`/`<Link>` — no special-casing needed (see the `NODE_ENV` note below for why an earlier revision briefly, incorrectly, thought otherwise).
- **A non-standard `NODE_ENV` in the deployment environment can crash the entire production build** with a misleading error: `next build` normally forces `NODE_ENV=production` itself and ignores `.env`'s value, but if the *shell/CI environment* already has `NODE_ENV` set to something outside `production`/`development`/`test` before Next.js starts, Next.js's own build-time env handling can end up inconsistent between its two internal render passes (the initial static-generation attempt and Turbopack's prerendering of the framework's own internal `/_global-error`/`/_not-found` pages). Symptom: `Cannot read properties of null (reading 'useContext')` while "Generating static pages", often paired with a wall of `Each child in a list should have a unique "key" prop` warnings on framework-internal elements (`<meta>`, `<head>`, `<html>`, `<script>`, `<__next_viewport_boundary__>`) — both are red herrings that look like an app-code or React-context bug but aren't. **The fix is entirely environmental, not a code change**: ensure `NODE_ENV` is unset or a standard value (`production`/`development`/`test`) in whatever shell/CI/platform actually runs `next build` — confirmed fixed once `NODE_ENV=production` was set correctly for the build in this project's case. Don't chase this by stripping `useRouter()`/`<Link>` from `error.tsx`/`not-found.tsx` or otherwise fighting React context in those files — that was tried, made no difference, and was reverted. If this resurfaces, check the build environment's `NODE_ENV` first, before suspecting application code.
...
- **S3 is the only image storage provider — there is no local-disk fallback.** `lib/uploads/storage/s3-image-storage.ts` (`S3ImageStorage`), using `@aws-sdk/client-s3` against a Railway bucket (any S3-compatible endpoint works the same way — Railway doesn't have a proprietary storage SDK, its buckets just speak the standard S3 API, same as R2/MinIO/Spaces/Backblaze). Config: `S3_ENDPOINT`, `S3_BUCKET`, `S3_REGION`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` — **all server-only, no `NEXT_PUBLIC_` prefix on any of them**, read only inside `server-only`-guarded files, and **required in every environment including local dev** (`getImageStorage()`, `lib/uploads/storage/index.ts`, always constructs `S3ImageStorage`; there's no env-var switch anymore). An earlier `LocalImageStorage` implementation (writing to `public/images/`) was deleted once S3 replaced it — don't recreate it without checking with whoever owns this decision, it was a deliberate simplification, not an oversight.
  - **The bucket does NOT need to support public read access.** An earlier revision assumed it did — `getUrl()` returned a plain, permanent `${S3_PUBLIC_URL_BASE}/${storageKey}` URL and expected the bucket to be configured for public read at the dashboard level (this provider silently ignores a canned `ACL: 'public-read'` on `PutObject`, confirmed by testing — there is no ACL/signing logic in the app). That assumption broke outright against a **Railway Storage Bucket**: Railway's own docs are explicit that its buckets are private-only ("no public buckets currently supported... direct public read access and ACLs are not available"), so images uploaded there could never be viewed at a bucket-direct URL, full stop — no dashboard setting fixes it. Fixed by adding `read(storageKey)` to the `ImageStorage` interface (an authenticated `GetObjectCommand`) and making `getUrl()` return `/uploads/${storageKey}` — `app/uploads/[...path]/route.ts` calls `getImageStorage().read()` on every request instead of touching a filesystem or the bucket's own URL. `S3_PUBLIC_URL_BASE` no longer exists as an env var; don't reintroduce it.
  - **`SignatureDoesNotMatch` (HTTP 403) on every operation — including a brand-new `PutObjectCommand` that had never touched the bucket before — turned out to mean exactly what it says: the bucket's secret access key had been rotated/regenerated in Railway's dashboard.** Root-caused by testing three independent signing paths (the AWS SDK, a hand-written SigV4 signer with zero AWS SDK code, and the running app) against the same credentials — all three failed identically, which ruled out an SDK/library bug and pointed at the credentials themselves. A real dead end explored first and abandoned: AWS SDK v3's default request/response checksum computation is a genuine, documented compatibility gap with some non-AWS S3 providers, and `requestChecksumCalculation`/`responseChecksumValidation: 'WHEN_REQUIRED'` on the `S3Client` is AWS's own recommended workaround for it — but it was **not** the cause here (adding it changed nothing) and was removed again; don't re-add it speculatively without first ruling out a plain bad/rotated credential the way this incident was actually diagnosed. If `SignatureDoesNotMatch` recurs: confirm the exact `S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY` currently in Railway's bucket dashboard match what's set on the **app service** (not just the bucket resource) in every environment (local `.env` included) before suspecting code.
  - **Also watch for a stale local dev server holding the old credentials in memory.** Node/Next.js only reads `.env` once, at process startup — editing `.env` (e.g., after rotating a key) does nothing until the process is actually restarted, not just the file saved. A `pnpm dev`/`pnpm start` left running in a background terminal will keep authenticating with whatever was in `.env` when it started, and a *second* dev server started elsewhere can silently bind to the next free port (3001, etc.) while the browser is still pointed at the stale one on :3000 — "I restarted and it's still broken" was, in one real incident, exactly this: check `Get-Process node` / whatever is bound to the port in use before assuming the fix didn't work.
  - `lib/uploads/storage-key.ts`'s `extractStorageKeyFromUrl()` — used by every entity service (`category.service.ts`, `service.service.ts`, `banner.service.ts`, `admin.service.ts`) to derive a storage key from a stored `image`/`avatarUrl` string for cleanup — recognizes only the single `/uploads/` prefix `getUrl()` produces. This file is `server-only`-guarded and is **not** imported by any client component: an earlier revision made the (now-removed) `S3_PUBLIC_URL_BASE` a `NEXT_PUBLIC_` var specifically so 4 client-side admin forms (`CategoryForm.tsx`, `ServiceForm.tsx`, `BannerForm.tsx`, `ChangeAvatarDialog.tsx`) could call `extractStorageKeyFromUrl()` directly for an "immediately delete the image I just replaced, before saving" optimization — that leaked the bucket URL (not a secret, but unnecessary exposure) into the client bundle. Fixed by moving key extraction into `deleteUploadedImageAction()` (`app/actions/uploads.ts`, now takes a raw URL instead of a pre-computed key) — the client forms now only compare the current image URL against a `useRef` of the URL the form loaded with (plain string equality, no key/prefix knowledge needed) and pass the raw URL to the action. Same cleanup behavior, zero bucket-URL knowledge required client-side. Don't reintroduce a direct `extractStorageKeyFromUrl()` import into a client component — route through `deleteUploadedImageAction()` instead.
  - Any row saved with the old, pre-proxy S3 URL scheme (a direct bucket URL, from before this fix) will keep failing to load and won't be auto-cleaned-up on replace/delete (harmless no-op) — same "known limitation" pattern as the bullet below: re-upload through the admin panel to fix an individual image, no bulk migration performed.
  - **`getImageStorage()` (`lib/uploads/storage/index.ts`) is a lazy, cached accessor, not an eagerly-constructed module-level `export const`.** `S3ImageStorage` (`s3-image-storage.ts`) similarly defers its own `S3Client`/env-var validation into a private `#getConfig()` called on first real use, not at module scope. This was a real, reproduced bug: `upload.service.ts` (and therefore this whole provider) gets imported transitively by nearly every product/category/service/banner-related page, even purely read-only storefront ones — an eager `new S3ImageStorage()` at import time meant a missing/misconfigured `S3_*` var crashed Next's static page-data collection for the *entire build*, not just image-upload paths. Confirmed locally: `S3_BUCKET= S3_ENDPOINT= ... pnpm build` failed with `Failed to collect page data for /categories/[slug]` — a page with nothing to do with uploads — caused by `module evaluation` of `s3-image-storage.ts` throwing. On a real deployment (different bundler/minification behavior, Next's retry-as-dynamic-after-failed-static-attempt path) the same root cause can surface as a much more confusing generic React error (e.g. `Cannot read properties of null (reading 'useContext')`) instead of the clean, labeled `requireEnv()` message seen locally — don't be misled by a cryptic React-internals-looking error into assuming it's a framework/hosting bug; check whether a module-level singleton somewhere is doing eager, env-dependent construction first. With the lazy fix, a genuinely missing/wrong `S3_*` var now only fails the specific upload attempt at runtime (clear error), never the build.
  - **Known limitation, not yet resolved**: pre-existing `Category`/`Banner`/`Product` rows created before S3 was wired up may still reference images that were never migrated into the bucket (this checkout has no access to wherever those files previously lived). New uploads go to the bucket; old ones keep failing to load until someone re-uploads them through the admin panel, or runs an equivalent migration script from an environment that actually has the old files.
